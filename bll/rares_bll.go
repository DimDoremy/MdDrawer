...
func GetAllRares() dto.ResultDto {
	var jsonResult dto.ResultDto
	data, err := dal.GetAllRares()
	if err != nil {
...
func GetRaresByWhere(column string, args []string) dto.ResultDto {
	var jsonResult dto.ResultDto
	var tmps []interface{}
	for _, v := range args {
		tmps = append(tmps, v)
	}
	data, err := dal.GetRaresByWhere(column, tmps...)
	if err != nil {
		common.App.Logger().Warn("Engine load failed:", err)
		jsonResult.RtnMsg = err.Error()
		jsonResult.RtnCode = 1
